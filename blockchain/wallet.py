...
def new_wallet(private_key=None,
               bits=2048,
               node=None):
    """
    ...
    return Wallet(private_key=private_key, node=node)
...
class Wallet(Base):
    ...
    def submit_transaction(self, **transaction):
        """
        Submits a transaction to the associated node.  If no node is configured,
        ...
        """
        if not self.node:
            raise ValueError('node must be configured to submit a transaction.')
        url = self.node + '/chain/block'
        return r.post(url=url, json=self.generate_transaction(**transaction))